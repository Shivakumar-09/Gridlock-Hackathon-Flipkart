...
print("\n--- Missing values (%) ---")
mv = pd.DataFrame({
    "train%": (train.isnull().sum() / len(train) * 100).round(2),
    "test%":  (test.isnull().sum()  / len(test)  * 100).round(2),
})
print(mv[mv.max(axis=1) > 0])
...
for col in train.columns:
    print(f"  {col:25s}: {train[col].nunique()}")
...
def kfold_te(train_df, test_df, group_cols, target=TARGET, n_folds=N_TE_FOLD, agg="mean"):
    """
    Leakage-safe target encoding for single or composite group.
    Returns (train_encoded_array, test_encoded_array, column_name).
    """
...
for df in [train, test]:
    ...
    df["landmark_congestion"] = (
        df["landmarks_bin"] * df[gh_hour_col] * (1 + df["peak_hour"])
    )
